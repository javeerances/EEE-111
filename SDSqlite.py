...
def testSDButtons():
    testing = SDSqlite(SDName='SDBSql.db')
    
    for data in range(30):
        testing.addschool(data, f'Name{data} Surname{data}', f'Email{data}', f'School{data}', 'Region I')
        assert testing.studentexists(data)

    alldata = testing.compiledata()
    assert len(alldata) == 30

    for data in range(10,20):
        testing.updateschool(f'Name{data} Surname{data}', f'Email{data}', f'School{data}', 'Region II', data)
        assert testing.studentexists(data)
    ...
    for data in range(10):
        testing.deleteschool(data)
        assert not testing.studentexists(data)

    alldata= testing.compiledata()
    assert len(alldata) == 20